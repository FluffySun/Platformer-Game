class Level{
  ...
  void loadMapFromImage(PImage img){
    ...
    for(int y=0;y<mapH;y++){
      for(int x=0;x<mapW;x++){
        ...
        if(c==color(0)) tiles[y][x]=TILE_SOLID;
      ...
      for(int x=0;x<mapW;x++){
        if(tiles[y][x]==TILE_BRIDGE && bridgeTriggered[y][x]){
          ...
          if(bridgeTimer[y][x]>30){
            tiles[y][x]=TILE_EMPTY; // falls away
          }
        }
      }
    }
  ...
  void drawTiles(){
    ...
    for(int y=0;y<mapH;y++){
      for(int x=startCol;x<endCol;x++){
        ...
        if(t==TILE_SOLID) image(groundTile, sx, sy, tileSize, tileSize);
        else if(t==TILE_GOAL) image(goalTile, sx, sy, tileSize, tileSize);
        else if(t==TILE_CHECKPOINT) image(checkpointTile, sx, sy, tileSize, tileSize);
        else if(t==TILE_PORTAL) image(portalTile, sx, sy, tileSize, tileSize);

        else if(t==TILE_LAVA) image(lavaFrames[lavaFrame], sx, sy, tileSize, tileSize);
        else if(t==TILE_TRAMPOLINE) image(trampolineImg, sx, sy, tileSize, tileSize);
        else if(t==TILE_ICE) image(iceImg, sx, sy, tileSize, tileSize);
        else if(t==TILE_SPIKE) image(spikeImg, sx, sy, tileSize, tileSize);
        else if(t==TILE_WATER) image(waterFrames[waterFrame], sx, sy, tileSize, tileSize);

        else if(t==TILE_TREE){
          // no collision, just draw stacked
          ...
          // choose e/center/w pattern by repeating every 3 tiles
          int mod = x % 3;
          ...
          // rails are decorative (no collision): draw above bridge
  ...
  boolean isIce(float x,float y){ return getTileAt(x,y)==TILE_ICE; }
  boolean isTrampoline(float x,float y){ return getTileAt(x,y)==TILE_TRAMPOLINE; }
